  ...
  return (
    <AxisWrap>
      ...
      <BarTrack>
        ...
        {isPositive ? (
          // 진보: 중앙에서 왼쪽으로
          <BarFill style={{ right: "50%", width: shown ? `${fillWidth}%` : "0%", background: "#3182f6", transitionDelay: `${delay}ms` }} />
        ) : (
          // 보수: 중앙에서 오른쪽으로
          <BarFill style={{ left: "50%", width: shown ? `${fillWidth}%` : "0%", background: "#e5484d", transitionDelay: `${delay}ms` }} />
        )}
      </BarTrack>
    </AxisWrap>
  );
